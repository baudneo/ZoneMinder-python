...
syslog_fmt = logging.Formatter("%(levelname)s [%(processname)s] [%(message)s]")
logger.addHandler(console_handler)


def metadata_cache(metadata: Optional[MetaData] = None, file=None):
    ...
    if file:
        try:
            ...
            logger.warning(
                f"The 'file' ({file}) passed to MetaDataCache is invalid, using default"
            )
...
class ZMSession:
    ...
    def __init__(self, options: Union[DBOptions, APIOptions], _type: str = None):
        ...
        if self.type == "db":
            ...
            logger.debug(
                f"After loading environment variables, db_config is: {db_config}"
            )
            ...
            if Path(f'{db_config["conf_path"]}/conf.d').exists():
                ...
                conf_data = read_zm_confs(files)
            else:
                logger.warning(
                    f"Use the option 'conf_path' or set the environment variable PYZM_CONF_PATH to specify "
                    f"the path to the ZM configuration files"
                )
                raise FileNotFoundError(
                    f"The ZM configuration path 'conf_path' ({db_config['conf_path']})"
                    f" does not exist!"
                )
            if conf_data:
            ...
            logger.debug(
                f"After loading {len(files)} ({files}) ZM config files, db_config is: {db_config}"
            )
            if (
            ...
            show_conn_str = conn_str.replace(
                db_config["dbpassword"], "<sanitized>"
            ).replace(db_config["dbhost"], "<sanitized>")
            try:
                logger.debug(
                    f"Connecting to ZM DB: {show_conn_str if options.sanitize else conn_str}"
                )
                ...
                self.metadata = metadata_cache(metadata=MetaData(engine))
                ...
                for attr in dir(self.auto_map.classes):
                    if attr.startswith("_") or not attr[0].isupper():
                        continue
                    setattr(
                        self.db, attr, (value := getattr(self.auto_map.classes, attr))
                    )
